...
public class Lane extends Thread implements PinsetterObserver {	
	...
	public int angle,velocity,position;
	 ...
	 */
	public void gaming(int an,int ve,int po){
		angle=an;
		velocity=ve;
		position=po;
		throw_flag=true;
	...
	private void gameNotFinished() {
		...
		if (bowlerIterator.hasNext()) {
			...
			while (canThrowAgain) {
				...
				setter.ballThrown(angle,velocity,position);		// simulate the thrower's ball hiting
				ball++;
				throw_flag=false;
				}
			}
	...
	private void additionalGame() {
		int n= party.getMembers().size() ;
		if(n<2) {
			System.out.println("No additional game, it's a single member team\n winner is = "+ ((Bowler)party.getMembers().get(0)).getNickName());
		}
		else {
			System.out.println("Additional game is running between second highest player and the highest player: ");
		    ...
		    for (int i = 0; i < n; ++i) {
		            ...
		            else if (finalScores[i][0] > sv && finalScores[i][0] <fv) {
		            	sv = finalScores[i][0];
		                second = i;
		            }
		    }
...
//		    System.out.println(first+" "+second);
		    ...
		    TieBreakGamePrompt tbg = new TieBreakGamePrompt((Bowler) party.getMembers().get(first), 
		    		(Bowler) party.getMembers().get(second),((Bowler)party.getMembers().get(0)).getNickName());
			int result = tbg.getResult();
			if(result==1) {
				tbg.distroy();
				tbg = null;
//				System.out.println(finalScores[first][0]+" "+finalScores[second][0]);
				TieBreakGame t = new TieBreakGame((Bowler) party.getMembers().get(first), (Bowler) party.getMembers().get(second),
						finalScores[first][0],finalScores[second][0]);
				t.duplicateGame();
				if(t.secondHighest<t.highestScore){
					System.out.println("Winner is = "+((Bowler) party.getMembers().get(first)).getNickName());
				}
				else{
					System.out.println("Winner is = "+((Bowler) party.getMembers().get(second)).getNickName());

				}
				cumulScores[second][9] = finalScores[second][0] = t.secondHighest;
				cumulScores[first][9] = finalScores[first][0] = t.highestScore;
		...
		for(int i=0; i<n; i++){
			...
			Date date = new Date();
			String dateString = "" + date.getHours() + ":" + date.getMinutes() + " " + date.getMonth() + "/" + date.getDay() + "/" + (date.getYear() + 1900);
			ScoreHistoryFile.addScore(((Bowler)party.getMembers().get(i)).getNickName(), dateString, new Integer(cumulScores[i][9]).toString());
			} catch (Exception e) {System.err.println("Exception in addScore. "+ e );} 	
		}
