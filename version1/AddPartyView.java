...
public class AddPartyView implements ActionListener, ListSelectionListener {
	...
	private JFrame win;
	private JButton addPatron, newPatron, remPatron, finished,remdb;
	...
	public AddPartyView(ControlDeskView controlDesk, int max) {
		...
		colPanel.setLayout(new GridLayout(_1, _3));
		...
		partyList.setFixedCellWidth(_120);
		partyList.setVisibleRowCount(_5);
		...
		allBowlers.setVisibleRowCount(_8);
		allBowlers.setFixedCellWidth(_120);
		...
		buttonPanel.setLayout(new GridLayout(_4, _1));

		Insets buttonMargin = new Insets(_4, _4, _4, _4);
		...
		remdb = new JButton("Remove patron");
		JPanel rempanel = new JPanel();
		rempanel.setLayout(new FlowLayout());
		remdb.addActionListener(this);
		rempanel.add(remdb);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(remdb)) {
			removeplayer rem = new removeplayer(this);
		}
		if (e.getSource().equals(finished)) {
			if ( party != null && party.size() > _0) {
 ...
 */
	...
	public void removePatron(removeplayer Patron) {
		try {
			Bowler checkBowler = BowlerFile.getBowlerInfo( Patron.getNick() );
			if ( checkBowler == null ) {
				System.err.println( "A player with that nick name doesnot exists." );
				
			} else {
				
				BowlerFile.removeBowler(Patron.getNick());
				bowlerdb = new Vector(BowlerFile.getBowlers());
				allBowlers.setListData(bowlerdb);
				party.remove(Patron.getNick());
				partyList.setListData(party);
			}
		} catch (Exception e2) {
			System.err.println("File I/O Error");
