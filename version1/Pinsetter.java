...
public class Pinsetter {
	 ...
	 */
	public void ballThrown(int speed,int angle,int position) {	// simulated event of ball hits sensor
		int count = 0;
		
		if(angle>60 || angle<-60 || (position==2 && angle>0) || (position==-2 && angle<0))
			count=0;
		else if(angle >-30 && angle <30 && speed >4 && speed <8 && position>=-1 && position<=1 ){

			double exp=(0.3*10) +( 0.7*rnd.nextInt(9));
			foul = false;
			double skill = 0.6;
		...
		else if (angle == 0 && position>=-1 && position<=1 ){
			double exp=( 0.5 *rnd.nextInt(10) )+(0.5*rnd.nextInt(9));
			double skill=0.5;
		...
		else if (speed > 0 && position>=-1 && position<=1 ){
			double exp=( 0.5 *rnd.nextInt(10) )+(0.5*rnd.nextInt(9));
			double skill=0.4;
